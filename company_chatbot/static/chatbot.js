...
function addGlobalStyles() {
    ...
    styleElement.textContent = `
        ...
        /* Updated blur overlay to cover the entire widget and darken more */
        .blur-overlay {
            position: absolute;
            top: 0;
            left: 0;
...
function showClearPopupInsideChat() {
    ...
    let existingPopup = chatWidget.querySelector('.chat-popup');
    if (existingPopup) existingPopup.remove();
    
    // Create blur overlay for the entire chat widget
    const blurOverlay = document.createElement('div');
    blurOverlay.classList.add('blur-overlay');
    chatWidget.appendChild(blurOverlay); // Attach to chatWidget instead of chatBox
    ...
    `;
    chatWidget.appendChild(popup); // Attach to chatWidget
