...
from process_resume import process_resume_folder
import re
import os
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        <div class="welcome-message">
            Hi there! 👋 <strong class="highlight">I'm BiNi LogBinary's virtual assistant</strong>. How can I help you today?
        </div>
        """
                ...
                if data.startswith('{') and data.endswith('}'):
                    try:
                        ...
                        if message_data.get("action") == "submit_inquiry":
                            ...
                            # Ensure the resume file is saved to the folder
                            resume_filename = application_data.get("resume_file", "")
                            resume_path = os.path.join("resumes", resume_filename)  # Save to 'resumes' folder
                            ...
                            if os.path.exists(resume_path):
                                process_resume_folder(folder_path="resumes", write_to_sheets=True, upload_to_drive=True)
                                logger.info(f"Processed resume: {resume_filename} and updated Google Sheet")
